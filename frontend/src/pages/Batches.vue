		...
		<div v-if="batches.data" class="pb-5">
			...
			>
				...
				<template #default="{ tab }">
					...
					<div v-else class="p-5 italic text-gray-500">
						{{ __('Нет {0} партий').format(tab.label.toLowerCase()) }}
					</div>
				</template>
			</Tabs>
			<div
					...
					<div class="px-5 py-32 rounded-md bg-gray-50">
						<div class="flex flex-col items-center space-y-2 text-center">
							...
							<span class="text-sm leading-4 text-gray-700">
								{{ __('Вы можете связать с ним курсы и оценки.') }}
							</span>
						</div>
					</div>
				</router-link>
			...
			>
				...
				<div>
					{{
						__(
							'В настоящее время партии недоступны. Следите за новостями, скоро появятся новые возможности для обучения!',
						)
					}}
				</div>
			</div>
